...
async def start_quiz_callback(callback_query: types.CallbackQuery):
    bot = callback_query.bot
    chat_id = callback_query.from_user.id
    await callback_query.answer()  # Закрываем "часики" на кнопке
    await start_quiz(chat_id, bot)
...
async def process_quiz_answer_callback(callback_query: types.CallbackQuery):
    bot = callback_query.bot
    await process_quiz_answer(callback_query, bot)
...
async def start_quiz_message_handler(message: types.Message):
    await start_quiz(message.chat.id, message.bot)

def register_quiz_handlers(dp: Dispatcher, bot: Bot):
    dp.register_callback_query_handler(
        process_quiz_answer_callback,
        lambda c: c.data.startswith("quiz:")
    )
    dp.register_callback_query_handler(
        start_quiz_callback,
        lambda c: c.data == "menu:quiz"
    )
    dp.register_message_handler(
        start_quiz_message_handler,
        commands=["quiz"]
    )
